...
fn fourteenthCheck(coefficients: l_coefficients, spoiled_ballot(x1, t_contests): spoiled_ballot, guards: guardians, missing_guards: guardians) -> bool {
    // 14.A
    // Set q
    ...
    for j in 0..missing_guards.len() {
        ...
        for k in 0..t_contests.len() {
            let (x8, d_contest) = t_contests[k].clone();
            let spoiled_ballot_decrypted_contest(x9, selects, x23) = d_contest;
            for m in 0..selects.len() {
                let (x10, select_tally) = selects[m].clone();
                let spoiled_ballot_decrypted_selection(x11, x12, x13, x14, shares) = select_tally;
