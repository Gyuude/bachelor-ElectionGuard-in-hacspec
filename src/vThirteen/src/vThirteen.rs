...
fn thirteenthCheck(spoil_bs: Seq<spoiled_ballot>, context(x21, x22, x23, x24, x25, x26, cebh, x27, x28): context, guards: Seq<guardian>) -> bool {
    ...
    for sbs in 0..spoil_bs.len() {
        ...
        for j in 0..contests.len() {
            let (x3, spoiled_ballot_decrypted_contest(x4, selections, x2300)) = contests[j].clone();
